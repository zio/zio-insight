package zio.insight.frontend.model

import scala.scalajs.js

import zio._
import zio.insight.ClientMessage
import zio.insight.frontend.utils.DomUtils.Color
import zio.insight.frontend.utils.Implicits._
import zio.metrics._

final case class TimeSeriesKey(metric: MetricKey.Untyped, subKey: Option[String] = None) {
  val key: String = metric.longName + subKey.map(s => s" - $s").getOrElse("")
}

/**
 * A time series key uniquely identifies a single line within a diagram view. For Counters and Gauges it can be directly
 * derived from the MetricKey, for the compound metrics it also requires a sub key to identify the bucket, the quantile
 * or the the set element.
 */
final case class TimeSeriesEntry private (key: TimeSeriesKey, when: js.Date, value: Double)
...
/**
 * The configuration for a single time series graph within a diagram. A single time series is either coming from a
 * counter, a gauge or represents a single element of a histogram, summary or set. For the latter a subkey must be
 * defined, which element is relevant for the time series.
 */

final case class TimeSeriesConfig(key: TimeSeriesKey, color: Color, tension: Double)
