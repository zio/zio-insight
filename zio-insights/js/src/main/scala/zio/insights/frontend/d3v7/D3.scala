...
package object d3v7 {
  ...
  import zio.insights.frontend.d3v7.d3Selection.Selection
  ...
  implicit class D3SelectionOps[T](self: Selection[T]) {
    ...
    /**
     * Convenience method for removing elements from a selection It will return a handle to the modified selection to
     * allow for a fluid DSL.
     */
    ...
    /**
     * Convenience method for adding something to a selection and then return a handle to the same collection. Useful
     * for adding multiple groups on the same level (as document siblings)
     */
